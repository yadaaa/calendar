...
app.service('CalendarService', function(DavClient, StringUtility, XMLUtility, CalendarFactory, WebCal) {
	'use strict';
	...
	this.get = function(url) {
		return context.bootPromise.then(function() {
			const url = DavClient.buildUrl(url);
			const depth = 0;
			const headers = {
				'requesttoken': OC.requestToken
			};
	...
	this.create = function(name, color, components=['vevent', 'vtodo']) {
		return context.bootPromise.then(function() {
			...
			return DavClient.request(method, url, headers, xml).then(function(response) {
				...
				return context.self.get(url).then(function(calendar) {
					// mitigate some issue in the Nextcloud server,
					// that doesn't allow storing custom properties when creating calendars
					calendar.enabled = true;
					return context.self.update(calendar);
				});
			});
		});
	};
	...
	this.publish = function(calendar) {
		const [skeleton] = XMLUtility.getRootSkeleton('o:publish-calendar');
	...
	this.unpublish = function(calendar) {
		const [skeleton] = XMLUtility.getRootSkeleton('o:unpublish-calendar');
